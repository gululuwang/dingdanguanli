...
export function createTranslateLoader(http: HttpClient,version:string) {
  ...
  return new TranslateHttpLoader(http, '/apps/fssp/fssc/web/bo-robxdjchenshjfront/fybxdjcustomqueryresult/i18n/', '.json'+ versionSuffix);
}

export let lang = {"zh-CHS":{"root-component":"","qdp-framework-component":"报销单自定义查询对象","spreadSheet1":""}};
    ...
    if (langCode == "zh-CHS" || (this.translate.defaultLang === langCode && this.translate.currentLoader == createTranslateLoader(this.http,null))) {
      ...
      const httpOb = this.http.get("/apps/fssp/fssc/web/bo-robxdjchenshjfront/version.json?v=" + new Date().getTime()).pipe(switchMap((data)=>{
        ...
        if (data instanceof Array) {
          const versionKey = "fybxdjcustomqueryresult/" + langCode + ".json";
          data.forEach((item) => {
